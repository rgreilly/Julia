...
# ╔═╡ d41bf003-4b98-4543-bc37-bc17eec3696b
begin
	using Flux
	using Flux: onecold
	using Plots
...
# ╔═╡ 736ca190-f0f4-4de7-8120-cb15d5f30371
HTML("""
<!-- the wrapper span -->
<div>
...
# ╔═╡ 29958d31-1dbe-4c93-8c9c-0ff8d535728e
md"""
Load necessary packages
"""

# ╔═╡ b8c1ce53-2f70-4ca9-b27e-f1a6cdfd2fd7
md"""
Use the `PlutoTest` library to test defined functions.
"""

# ╔═╡ 62ab7747-f5a4-45e2-a2ef-87850c4243d1
function charToMorseVector(c::Char)
    # Define the mapping from characters to Morse code
    morseDict = Dict(
        'A' => ".-",   'B' => "-...", 'C' => "-.-.", 'D' => "-..",
        'E' => ".",    'F' => "..-.", 'G' => "--.",  'H' => "....",
        'I' => "..",   'J' => ".---", 'K' => "-.-",  'L' => ".-..",
    ...
    if haskey(morseDict, c)
        # Convert the Morse code to the numerical vector format
        morseCode = morseDict[c]
        morseVector = [0.5] # Start with the first element fixed at 0.5
        for symbol in morseCode
            if symbol == '.'
                push!(morseVector, 0.0)
            elseif symbol == '-'
                push!(morseVector, 1.0)
            end
        end
        # Pad the vector with 0.5 to ensure it has 5 elements
        while length(morseVector) < 5
            push!(morseVector, 0.5)
        end
        return morseVector
    else
        error("Input must be an alphabetic character.")
    end
end

# ╔═╡ b7cd55d0-5ebd-4a19-afc8-f37337b7b285
@test charToMorseVector('A') == [0.5, 0.0, 1.0, 0.5, 0.5]

# ╔═╡ 56101413-6ef7-4709-b821-c0d7ff402a34
function generate_morse_sequence(label::Char)
	seq = charToMorseVector(label)
	seq, label
end

# ╔═╡ 88f7c198-0270-4da6-8248-9c91e86fbd18
function generate_morse_sequence_vect(labels::Vector{Char})
	seq = vcat([charToMorseVector(x) for x ∈ labels]...)
	seq, labels
end

# ╔═╡ fcaac785-5827-4955-bc4a-eed215cace26
@test generate_morse_sequence('c') == ([0.5, 1.0, 0.0, 1.0, 0.0], 'c')
...
begin
	function randomCombinations(N::Int)
	    # Generate N permuted alphabet sequences
	    alphabet = collect('a':'z')
	    permutations = [shuffle(alphabet) for _ in 1:N]
	    
	    # Generate combinations
		comb_seq = Vector{Vector{Char}}(undef, 26)
	    for i in 1:length(alphabet)
			comb = Vector{Char}(undef,N)
			for j in 1:N
	        	comb[j] = permutations[j][i]
	        end
			comb_seq[i] = comb
	    end 
	    return comb_seq
...
function generate_morse_sequence_batch(nseq::Int, lgth::Int)
	# Initialise empty arrays for sequences and labels
	seqs = Vector{Vector{Float32}}(undef, 26nseq)
	labs = Vector{Vector{Char}}(undef, 26nseq)

	# Fill arrays
	i = 1
	for j in 1:nseq
		comb_seq = randomCombinations(lgth)
		for c ∈ comb_seq
			seqs[i], labs[i] = generate_morse_sequence_vect(c)
			i += 1
		end
	end
	# Encode sequences using one-hot encoding and return
	labs = map(x -> Flux.onehot(x, collect('a':'z')), vcat(labs...))
	seqs, labs
end
...
manifest_format = "2.0"
project_hash = "6b777eff727a080ee8f04eac1c3263fe12f136f5"
